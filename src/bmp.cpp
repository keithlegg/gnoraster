...
// void BMP::uint32_t make_stride_aligned(uint32_t align_stride) 
// {
...
void BMP::dump_rgba_data(uint32_t x0, uint32_t y0, uint32_t w, uint32_t h, framebuffer::RGBType *rgbdata) {
    if (x0 + w > (uint32_t)bmp_info_header.width || y0 + h > (uint32_t)bmp_info_header.height) {
        throw std::runtime_error("The region does not fit in the image!");
    }
