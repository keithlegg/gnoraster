...
// Candidate fiducials are placed in instances of this struct...
struct fiducial{
	int id = 0; //not used YET but its a good idea to have
	bool is_angled_checked = false; //we need to only check fiducials ONCE for angles 
    int confidence;
    int x = 0;
    int y = 0;
};
...
fiducial newfid( int id, int x, int y, int confidence );
